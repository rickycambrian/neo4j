...
public class RoleSecurityGraphComponent {
    ...
    public void grantPrivilege(Transaction tx, String roleName, Privilege privilege) {
        ...
        try (ResourceIterator<Node> roleNodes = tx.findNodes(Label.label(ROLE_LABEL), ROLE_NAME, roleName)) {
            if (roleNodes.hasNext()) {
                ...
                for (Relationship rel : roleNode.getRelationships(Direction.OUTGOING, RelationshipType.withName(HAS_PRIVILEGE))) {
    ...
    private Node createPrivilegeNode(Transaction tx, Privilege privilege) {
        ...
        privNode.setProperty("relationshipTypes", privilege.resource().relationshipTypes().toArray(new String[0]));
    ...
    private Privilege nodeToPrivilege(Node privNode) {
        try {
            Privilege.PrivilegeAction action = Privilege.PrivilegeAction.valueOf((String) privNode.getProperty("action"));
            ...
            Privilege.PrivilegeResource resource = new Privilege.PrivilegeResource(graph, labels, types, props, pattern);
