...
public class RBACValidation {
    ...
    private static final Set<String> RESERVED_ROLES = Set.of(
        "PUBLIC",
        "SYSTEM",
        "ANONYMOUS"
    );
        ...
        if (roleName.length() < MIN_ROLE_NAME_LENGTH || roleName.length() > MAX_ROLE_NAME_LENGTH) {
            throw new InvalidArgumentException(
                String.format("Role name must be between %d and %d characters", 
                    MIN_ROLE_NAME_LENGTH, MAX_ROLE_NAME_LENGTH));
        }
    ...
    public static void validateRoleDeletion(String roleName) {
        if (RESERVED_ROLES.contains(roleName.toUpperCase())) {
            throw new InvalidArgumentException(
                String.format("Cannot delete reserved role '%s'", roleName));
        }
        
        if (SYSTEM_ROLES.contains(roleName)) {
            throw new InvalidArgumentException(
                String.format("Cannot delete system role '%s'", roleName));
        }
    }
    ...
    public static void validateRoleModification(String roleName) {
        if (RESERVED_ROLES.contains(roleName.toUpperCase())) {
            throw new InvalidArgumentException(
                String.format("Cannot modify reserved role '%s'", roleName));
        }
    }
    ...
    public static void validatePrivilegeAction(String action) {
        ...
        try {
            ...
            throw new InvalidArgumentException(
                String.format("Invalid privilege action: '%s'", action));
        }
    }
    ...
    public static void validatePrivilegeScope(String scope) {
        ...
        try {
            ...
            throw new InvalidArgumentException(
                String.format("Invalid privilege scope: '%s'", scope));
        }
    }
    ...
    public static void validateLabels(Set<String> labels) {
        ...
        for (String label : labels) {
            ...
            if (!label.equals("*") && !label.matches("^[a-zA-Z0-9_]+$")) {
                throw new InvalidArgumentException(
                    String.format("Invalid label: '%s'. Labels can only contain letters, numbers, and underscores", label));
            }
        }
    }
    ...
    public static void validateRelationshipTypes(Set<String> types) {
        ...
        for (String type : types) {
            ...
            if (!type.equals("*") && !type.matches("^[a-zA-Z0-9_]+$")) {
                throw new InvalidArgumentException(
                    String.format("Invalid relationship type: '%s'. Types can only contain letters, numbers, and underscores", type));
            }
        }
    }
    ...
    public static void validateProperties(Set<String> properties) {
        ...
        for (String property : properties) {
            ...
            if (!property.equals("*") && !property.matches("^[a-zA-Z0-9_]+$")) {
                throw new InvalidArgumentException(
                    String.format("Invalid property: '%s'. Properties can only contain letters, numbers, and underscores", property));
            }
        }
    }
     ...
     */
    public static void requirePrivilege(Set<Privilege> userPrivileges, Privilege.PrivilegeAction requiredAction,
                                       String errorMessage) {
        boolean hasPrivilege = userPrivileges.stream()
            .anyMatch(p -> p.granted() && p.action() == requiredAction);
